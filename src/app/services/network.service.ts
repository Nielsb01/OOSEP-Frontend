...
export class NetworkService {
  ...
  public post(url: string, data: any, appendUserId: boolean = true): Observable<any> {
    return this.sendToBackend(url, appendUserId,(completeUrl, observer) => {
      this.httpClient.post(completeUrl, data, this.httpOptions).subscribe(observer);
    });
  }

  private sendToBackend(url: string, appendUserId: boolean, callback: (completeUrl: string, observer) => any): Observable<any> {
